...
make d       # dev-сервер
make tables  # пересобрать таблицы jsonl из четырёх каталогов
make check   # критики данных, типы, линтеры, тесты, покрытие
```

## Таблицы

Каноническая реляционная модель описана в [docs/intake/data-model.md](./docs/intake/data-model.md). Пока базы нет, каждая таблица лежит файлом `data/<table>.jsonl`: одна строка это один кортеж, ключи совпадают с именами столбцов, значения только скалярные.

Таблицы порождаются из четырёх каталогов упражнений командой `make tables` и коммитятся: они уезжают вместе с деплоем. Критик таблиц проверяет пять правил формата, целостность внешних ключей и перенос каталогов, он входит в `make check`.
