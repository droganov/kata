...
export function sessionsOf(program: Program, exercises: PlanExercises): readonly Session[] {
...
function cycledOne<Item>(items: readonly Item[], cursor: Cursor, key: string): readonly Item[] {
	if (items.length === 0) return [];
	const at = stepOf(cursor, key) % items.length;
	return items.slice(at, at + FIRST_PICK);
}

function hipPlanePicks(
	program: Program,
	slot: Slot,
...
function isLoadedToday(picked: PickedSlots, slotId: string): boolean {
	const chosen = picked.get(slotId);
	return chosen !== undefined && chosen.length > 0;
}

function lateralPicks(
	slot: Slot,
	candidates: readonly PlanExercise[],
	cursor: Cursor
): readonly PlanExercise[] {
...
function sessionSectionOf(picked: PickedSection): SessionSection {
	return {
...
function sessionSlugOf(program: Program, index: number): string {
...
function stepOf(cursor: Cursor, key: string): number {
