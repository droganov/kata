...
export const derivedId = (parts: readonly string[]): string => {
...
const hashedWord = (text: string, seed: number): string => {
	let hash = seed;
...
const markedDigit = (digit: string, at: number): string => {
	if (at === VERSION_AT) return VERSION_MARK;
	if (at === VARIANT_AT) return variantOf(digit);
	return digit;
};

const variantOf = (digit: string): string =>
	VARIANT_MARKS.charAt(Number.parseInt(digit, HEX_RADIX) % VARIANT_MARKS.length);
