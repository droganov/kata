...
export const criticExitCode = (report: TableReport): number => (report.failureCount > 0 ? 1 : 0);

export const criticLines = (report: TableReport): string[] => [
	...report.tables.map(
		(table) => `${TABLE_LABEL}${table.name}${FIELD_GAP}${ROWS_LABEL}${String(table.rowCount)}`
	),
	...report.findings.map(
		(finding) =>
